function New-Entity {
    ...
    $url = "$($url.TrimEnd("/"))/DataMarts($id)/Entities"
    ...
    # attempt to add
    try {
        Write-DosMessage -Level "Information" -Message "Attemping to post json metadata for the ""$($clientObject.Name)"" entity with Metadata Service. POST:$url"
        ...
        if ($response) {
            Write-DosMessage -Level "Information" -Message "Successfully registered entity: $($response | Select-Object Id, Name | ConvertTo-Json -Compress)"
            return $response
        }
        else {
        ...
        if ((Assert-WebExceptionType -exception $exception -typeCode 400) -and ((Get-ErrorFromResponse -response $exception.Response) -match "'Name' already exists")) {
            Write-DosMessage -Level "Information" -Message """$($clientObject.Name)"" entity already registered with Metadata Service."
            $url = "$($url)?`$filter=Name eq '$($clientObject.Name)'"
            Write-DosMessage -Level "Information" -Message "Getting ""$($clientObject.Name)"" entity. GET:$url"
            $response = Invoke-RestMethod -Method Get -Uri $url -ContentType "application/json" -Headers $headers -UseBasicParsing
            if ($response.value) {
                $response = $response.value
                Write-DosMessage -Level "Information" -Message "Successfully found entity: $($response | Select-Object Id, Name | ConvertTo-Json -Compress)"
                return $response
            }
            else {
                Write-DosMessage -Level "Fatal" -Message "Something went wrong! The POST request succeeded, but the response was empty."
            ...
            Write-DosMessage -Level "Fatal" -Message "There was an error registering datamart ""$($clientObject.Name)"" with Metadata Service: $_error, halting installation."
        }
    }
}